...
module.exports = {
    ...
    sendProduct: async function (ctx, productId, product, iscart) {
        ...
        const formatTelegramMessage = (product) => {
            ...
            const foooormatedhighrlight= formattedHighlights&&`▫️ ${formattedHighlights}`
...
Markup.button.callback('🛒 Buy', `buy_${productId}`),
            ...
            try {
                if (image) {
                    ...
                    const message = await ctx.replyWithPhoto({ url: resizeimage }, {
                        caption: telegramMessage,
